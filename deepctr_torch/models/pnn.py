...
class PNN(BaseModel):
    ...
    def __init__(self, linear_feature_columns,
        ...
        if use_inner and use_outter:
            self.order_len = self.compute_order_dim(
                dnn_feature_columns, embedding_size)
            order_to_dnn = self.compute_order_dim(
                dnn_feature_columns, embedding_size) * 2
        ...
        if self.use_inner:
            self.innerproduct = InnerProductLayer(
                self.order_len, self.embedding_size, field_size, device=device)
            """

...
"""
Author:
    Zeqian Cheng, chengzeqian321@163.com
Reference:
    [1] Liu B, Tang R, Chen Y, et al. Feature Generation by Convolutional Neural Network for Click-Through Rate Prediction[J]. arXiv preprint arXiv:1904.04447, 2019.s(https://arxiv.org/pdf/1904.04447)
"""
import torch
...
from ..layers.interaction import FGCNNLayer, InnerProductLayer
...
class FGCNN(BaseModel):
    ...
    def __init__(self, dnn_feature_columns, conv_kernel_width=(7, 7, 7, 7), conv_filters=(14, 16, 18, 20),
                 new_maps=(3, 3, 3, 3), pooling_width=(2, 2, 2, 2), 
                 dnn_hidden_units=(128,), dnn_activation='relu', dnn_use_bn=False,
                 l2_reg_linear=1e-5, l2_reg_embedding=1e-5, l2_reg_dnn=0, dnn_dropout=0,
                 init_std=0.0001, seed=1024, task='binary', device='cpu'):
        
        super(FGCNN, self).__init__([], dnn_feature_columns, dnn_hidden_units=dnn_hidden_units,
        ...
        if not (len(conv_kernel_width) == len(conv_filters) == len(new_maps) == len(pooling_width)):
            raise ValueError(
                "conv_kernel_width, conv_filters, new_maps and pooling_width must have the same length")

        self.conv_filters = conv_filters
        self.conv_kernel_width = conv_kernel_width
        self.new_maps = new_maps
        self.pooling_width = pooling_width
        self.fg_embedding_dict = copy.deepcopy(self.embedding_dict)
        self.field_size = self.compute_input_dim(dnn_feature_columns, include_dense=False, feature_group=True)
        ...
        self.dnn = DNN(self.dnn_input_dim, dnn_hidden_units,
                       ...
                       dropout_rate=dnn_dropout, init_std=init_std, 
                       device=device)
    ...
    def forward(self, X):
        fg_sparse_embedding_list, _ = self.input_from_feature_columns(X, self.dnn_feature_columns,
                                                                      self.fg_embedding_dict)
        origin_sparse_embedding_list, _ = self.input_from_feature_columns(X, self.dnn_feature_columns, 
                                                                        self.embedding_dict)
        fg_input = torch.cat(fg_sparse_embedding_list, dim=1)
        origin_input = torch.cat(origin_sparse_embedding_list, dim=1)
